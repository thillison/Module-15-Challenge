...
def recommend_portfolio(intent_request):
    ...
    if source == "DialogCodeHook":
        ...
        #validate inputs
        validation_result = validate_info(first_name, age,investment_amount,risk_level,intent_request)
        ...
        # When all slots are valid, a delegate dialog is returned to Lex to choose the next course of action.
        return delegate(output_session_attributes, get_slots(intent_request))

    recommendation=get_recommendation(risk_level)
    
      if risk_level == 'none':
        advice = f"Thank you {first_name}, we recommend a portfolio that consists of 100% bonds (AGG), 0% equities (SPY)."
    elif risk_level == 'low':
        advice = f"Thank you {first_name}, we recommend a portfolio that consists of 60% bonds (AGG), 40% equities (SPY)."
    elif risk_level == 'medium':
        advice = f"Thank you {first_name}, we recommend a portfolio that consists of 40% bonds (AGG), 60% equities (SPY)."
    elif risk_level == 'high':
        advice = f"Thank you {first_name}, we recommend a portfolio that consists of 20% bonds (AGG), 80% equities (SPY)."
    ...
    return close(
        ...
        #validate inputs
        validation_result = validate_input(first_name, age,investment_amount,risk_level,intent_request)
    ...
    return close(
        ...
        {
            "contentType": "PlainText",
            "content": """We wish you the best on your investing journey, goodbye.""",
        },
    )
